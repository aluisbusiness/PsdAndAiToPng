...
function convert(quantity) {
  return {
    from: function from(fromUnit) {
      ...
      !units ? process.env.NODE_ENV !== "production" ? invariant(false, "No conversion could be found for " + from) : invariant(false) : void 0;
      var fromConversion = conversionRatio(units, from);
      return {
        to: function to(toUnit) {
          ...
          var toConversion = conversionRatio(units, to);
            ...
            if (process.env.NODE_ENV !== "production") {
              try {
                ...
                bigintValue = quantity * BigInt(combinedRatio) + (BigInt(fromConversion.difference) - BigInt(toConversion.difference));
                 ...
                 process.env.NODE_ENV !== "production" ? invariant(false, "Conversion for " + from + " to " + to + " can't be expressed as an integer") : invariant(false) ;
              }
            } else {
              bigintValue = quantity * BigInt(combinedRatio) + (BigInt(fromConversion.difference) - BigInt(toConversion.difference));
            }
...
var typedConvert = convert;

export { conversions, typedConvert as convert };
